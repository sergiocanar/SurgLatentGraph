...
train_dataloader = dict(
    batch_size=64,
    dataset=dict(
        ann_file='train/c1_1_annotation_ds_coco.json',
...
val_dataloader = dict(
    batch_size=64,
    dataset=dict(
        ann_file='val/c1_1_annotation_ds_coco.json',
    ),
)
test_dataloader = dict(
    batch_size=64,
    dataset=dict(
        ann_file='test/c1_1_annotation_ds_coco.json',
    ),
)
...
train_evaluator = dict(
    ...
    ann_file=os.path.join(_base_.data_root, 'train/c1_1_annotation_ds_coco.json'),
    ...
    ds_per_class=False,
    criteria=1,
    outfile_prefix='./results/sages_og_fold1_preds/train/lg',
)
val_evaluator = dict(
    type='CocoMetricRGD',
    prefix='sagesf1_c1',
    ...
    ann_file=os.path.join(_base_.data_root, 'val/c1_1_annotation_ds_coco.json'),
    use_pred_boxes_recon=True,
    metric=[],
    task_type='binary',
    ds_per_class=False,
    criteria=1,
    num_classes=1,
    outfile_prefix='./results/sages_og_fold1_preds/val/lg',
)
...
test_evaluator = dict(
    ...
    ann_file=os.path.join(_base_.data_root, 'test/c1_1_annotation_ds_coco.json'),
    metric=[],
    task_type='binary',
    num_classes=1,
    ds_per_class=False,
    ...
    use_pred_boxes_recon=True,
    outfile_prefix='./results/sages_og_fold1_c1_preds/test/lg',
)
...
default_hooks = dict(
    checkpoint=dict(save_best='sagesf1_c1/ds_average_precision'),
    visualization=dict(
...
# loading
load_from = 'weights/sages_og_fold1/lg_base.pth'
